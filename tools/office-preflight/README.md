...
- The fixed SQL order in `manifest.json`, each predecessor, all six direct migration filenames (including the base schema), unique 14-digit versions, ascending filename order, and the SHA-256 digest of each SQL file's bytes. Missing, changed or unlisted migrations fail.
