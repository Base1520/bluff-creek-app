...
Run `node tools/office-preflight/cli.mjs` separately against the reviewed checkout to verify all eight active SQL files and their recorded digests. Comparing the baseline manifest hash alone does not read or validate those migration bytes. The source preflight also remains a local check; compare the actual hosted history and grants separately during the authorized capture.
