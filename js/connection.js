  ...
  function initialize(doc, options) {
    ...
    function draw() {
      Object.values(modes).forEach(function (mode) {
        mode.fields.disabled = !client || !!mode.attempt || mode.busy || (mode.kind === 'guest' && (guestRemoved || reading));
        var button = el(mode.kind + '-submit');
        button.disabled = !client || mode.busy || (!user && (authBusy || authPending || authUncertain)) || (mode.kind === 'guest' && (guestRemoved || reading || (!!user && !loaded)));
    ...
    async function submit(mode) {
      ...
      try {
        ...
        if (mode.kind === 'guest') {
          ...
          status('guest-status', 'Registration receipt confirmed. Checking the current guest registration and welcome email status. This does not make you a church member or subscribe you to group messages.');
