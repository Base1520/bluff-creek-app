...
(function (root, factory) {
  ...
  // Keep only the public snapshot contract when a page retains its latest read.
  // The caller owns this in-memory value; no browser storage is used here.
  function publicSnapshot(data) {
    return {source:'icloud',updated:churchToday(new Date(data.synced_at)),synced_at:data.synced_at,
      valid_until:data.valid_until,recurring:[],events:data.events.map(function (event) {
        var clean = normalizeEvent(event);
  ...
  async function loadIcloud(options) {
    ...
    if (validSnapshot(options.previousSnapshot) && (!chosen || Date.parse(options.previousSnapshot.synced_at) >= Date.parse(chosen.synced_at))) {
      chosen = options.previousSnapshot; transport = 'memory';
    }
    ...
    chosen = chosen ? publicSnapshot(chosen) : null;
    var events = chosen ? upcomingEvents(chosen.events, today).filter(function (event) { return isUpcoming(event, today, minutes); }) : [];
    return { source:'icloud', events:events, snapshot:chosen, sources:{weekly:transport,community:'not-used'},
  ...
  function statusText(result) {
    if (result.source === 'icloud') {
      ...
      var saved = result.stale || result.sources.weekly === 'cache' || result.sources.weekly === 'memory';
      return (saved ? 'Saved church calendar from ' : 'Church calendar updated ') + date + '. All times Central.' +
        (saved ? ' Recent changes may be missing.' : '');
    }
