...
(function(root){
  ...
  function create(options){
    ...
    async function load(epoch){
      ...
      try{
        ...
        if(draft && !draft.busy){
          ...
          else if(!latest)draft.conflict=true;
          else if(draft.payload && latest.version>draft.version && Object.keys(draft.payload).every(function(k){return (latest[k]==null?null:latest[k])===draft.payload[k];})){close(true);options.notice('The refreshed action or route matches the submitted changes.');}
    ...
    function matches(row){return (filter==='all' || filter==='completed'&&row.status==='completed' || filter==='new'&&row.status==='new' || filter==='open'&&row.status!=='completed' || filter==='due'&&row.status!=='completed'&&row.due_on<=today()) && (!search.trim() || [sourceLabel(row),kinds[row.kind],statuses[row.status],staffLabel(row.assigned_staff_user_id),row.due_on,mail[row.notification_status]].join(' ').toLowerCase().includes(search.trim().toLowerCase()));}
    function render(){
      ...
      var routeArea=host.querySelector('[data-intake-routes]');routeArea.innerHTML='<h3>Where new submissions go</h3><p class="intake-note">Office is the default until an approved staff route is selected. Defaults apply to future submissions; existing tasks keep their own assignment. Only administrators change defaults. Staff notification status comes from the durable email queue.</p>';
      settings.routes.forEach(function(r){var line=doc.createElement('div');line.className='intake-route';line.innerHTML='<div><strong>'+safe(kinds[r.kind])+'</strong><p>'+safe(staffLabel(r.assigned_staff_user_id))+'</p></div>';if(context().role==='admin'){var edit=doc.createElement('button');edit.type='button';edit.className='quiet';edit.textContent='Change default';edit.dataset.intakeRoute=r.kind;edit.onclick=function(){open('route',r.kind);};line.appendChild(edit);}routeArea.appendChild(line);});syncEditor();
    }
    function refresh(){return options.refresh?options.refresh():load(context().epoch);}
    function syncEditor(){
      syncUnload();if(!draft||!dialog)return;var blocked=draft.busy||draft.pending||draft.requiresRefresh||draft.conflict||!ready||!current(draft.epoch,draft.owner)||(draft.mode==='route'&&context().role!=='admin');
      dialog.querySelectorAll('input,select').forEach(function(n){n.disabled=!!blocked||!!draft.payload;});dialog.querySelector('[type=submit]').disabled=!!blocked;dialog.querySelectorAll('[data-intake-close],[data-intake-retry]').forEach(function(n){n.disabled=!!draft.busy||!!draft.pending;});
    ...
    function open(mode,id){
      ...
      form.addEventListener('input',syncUnload);form.addEventListener('change',syncUnload);dialog.querySelectorAll('[data-intake-close]').forEach(function(b){b.onclick=close;});dialog.addEventListener('cancel',function(e){e.preventDefault();close();});dialog.querySelector('[data-intake-retry]').onclick=refresh;
      form.onsubmit=async function(event){
        event.preventDefault();if(draft!==state||state.busy||state.pending||state.requiresRefresh||state.conflict||!ready||!current(epoch,owner)||token!==formId||mode==='route'&&context().role!=='admin'||!form.reportValidity())return;
        var formData=values(),payload=state.payload||Object.assign({assigned_staff_user_id:formData.assigned_staff_user_id||null},mode==='task'?{due_on:formData.due_on,status:formData.status}:{}),error=dialog.querySelector('[role=alert]');
        if(payload.assigned_staff_user_id&&!settings.staff.some(function(p){return p.id===payload.assigned_staff_user_id;})||mode==='task'&&(!day(payload.due_on)||!Object.hasOwn(statuses,payload.status))){error.textContent='Choose Office or eligible staff, a valid due date and an action status.';return;}
        state.payload=payload;state.busy=true;error.textContent='';syncEditor();
        try{
          if(options.ensureReady&&!await options.ensureReady(epoch))throw new Error('readiness');if(!current(epoch,owner)||draft!==state||token!==formId||!ready||mode==='route'&&context().role!=='admin')throw new Error('readiness');
          state.pending++;state.revision++;var args=mode==='route'?{p_kind:id,p_version:state.version,p_assigned_staff_user_id:payload.assigned_staff_user_id}:{p_id:id,p_version:state.version,p_changes:payload};
          function settled(){state.pending--;state.revision++;if(draft===state&&!state.busy){state.requiresRefresh=true;syncEditor();}}
          var operation=rpc(mode==='route'?'set_intake_route':'update_intake_task',args).then(function(r){settled();return r;},function(e){settled();throw e;});var result=await deadline(operation);
          ...
          if(result.error||!result.data||result.data.version!==state.version+1||result.data[mode==='route'?'kind':'id']!==id)throw new Error('unconfirmed');
          close(true);await refresh();if(current(epoch,owner))options.notice(mode==='route'?'Default route saved for future submissions. Existing actions were not reassigned.':'Action saved. Check its separate notification status for email progress.');
