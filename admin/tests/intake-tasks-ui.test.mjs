...
test('attention task handoff explicitly reports missing, unavailable and refused drafts',async t=>{
 const f=fixture(t);assert.equal(f.api.openTask(task.id),false);await f.api.load(1);assert.equal(f.api.openTask('newer-than-loaded-snapshot'),false);assert.equal(f.api.openTask(task.id),true);change(f,'due_on','2030-01-01');f.w.confirm=()=>false;assert.equal(f.api.openTask(task.id),false);assert.equal(f.form().elements.due_on.value,'2030-01-01');assert.equal(f.writes().length,0);
});
