...
1. Use the dedicated **Bluff Creek Church Office** project in Base1520 (`xzfeumdonxeodqhfirjr`). Its seven-migration schema was applied after explicit approval on September 8; staff access, email and release remain pending. Do not create another project or use an unrelated database.
2. Compare the current seven-file manifest with hosted migration history before any further setup. All seven migrations are already applied to this project, with exact approved SQL hashes; do not rerun them. See the [migration history and activation guide](../docs/office-migrations.md) and [hosted verification](../docs/office-hosted-verification-2026-09-08.md). A new isolated rehearsal follows the manifest's explicit order; it does not change this hosted history.
