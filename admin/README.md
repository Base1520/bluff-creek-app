# Creek Office setup
For current setup, use the [ten-migration history](../docs/office-migrations.md) and [direct-intake contract](../tools/direct-intake/README.md). The earlier [activation sequence](../docs/office-hosted-activation.md) and [offline wiring tool](../tools/office-wiring/README.md) describe the historical staff-first configuration; the tool does not configure the current direct-intake release or apply schema, invite staff or enable a hosted service.
...
1. Use the dedicated **Bluff Creek Church Office** project in Base1520 (`xzfeumdonxeodqhfirjr`). Its original seven migrations were applied September 8, the account guard September 9 as `20260909124122`, and direct intake plus dispatcher extensions September 10 as `20260910152017` and `20260910152030`. Do not create another project or use an unrelated database.
2. Compare the current ten-file, format-4 manifest with hosted migration history before any further setup. All ten migrations are already applied to this project, with exact approved SQL hashes; verify them rather than rerunning them. The eighth guard remains required before staff grants in a fresh environment; see the [guard application record](../docs/office-account-guard-hosted-2026-09-09.md) and [current history map](../tools/office-preflight/direct-intake-hosted-history-2026-09-10.json). A new isolated rehearsal follows the manifest's explicit order; it does not change hosted history. Schema alignment alone does not certify staff sign-in, email delivery, phone behavior or backups.
...
5. Preserve the reviewed published configuration. Development-source configuration remains blank intentionally; published PR #7 supplies the approved browser configuration and enables direct intake. The historical [offline wiring tool](../tools/office-wiring/README.md) stages staff-first settings only and must not replace the current public configuration. `config.example.js` documents staff configuration. Only the reviewed project URL and modern **publishable** key belong in browser settings, never a secret or `service_role` key. Hosted login and recovery require a root HTTPS Supabase URL; legacy anon tokens are only supported by the explicit local rehearsal.
...
## App signup, private prayer and ongoing care

The published `connection.html` flow is separate from staff sign-in. It creates email/password accounts without requiring email confirmation, so email addresses and membership status are self-reported. Authenticated users can submit their own voluntary guest/household profile or a private prayer request; anonymous writes remain denied. New guest registration saves a Guest register entry, an Office follow-up task and a welcome-email job together. Prayer submission saves a private prayer and care task, with generic staff notices; prayer text and household details do not enter the email jobs. A saved submission confirms the record, while provider acceptance does not prove delivery. The flow grants neither staff access nor church membership, and does not automatically alter the membership ledger. See the [current intake and routing contract](../tools/direct-intake/README.md). Development-source configuration remains blank; the approved published PR #7 configuration enables this flow. Staff identity review remains explicit, and deacon and Sunday school plans keep separate quarterly/monthly deadlines.
