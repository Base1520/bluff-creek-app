...
(function (root) {
  ...
  function create(options) {
    ...
    var review = null;
    var ready = false, failed = false, dialog = null, mounted = false, mountedEpoch = null, mountedOwner = null, filter = 'pending', term = '';
    ...
    function clear() { request++; close(true); rows = []; ready = false; failed = false; mounted = false; mountedEpoch = null; mountedOwner = null; filter = 'pending'; term = ''; host.replaceChildren(); if (options.onCount) options.onCount(null); }
    function mount() {
      if (mounted) return;
    ...
    function syncReview() {
      ...
      var blocked=review.busy || review.requiresRefresh || review.conflict || !ready || !current(review.epoch, review.owner) || !peopleAvailable();
    ...
    function open(id) {
      ...
      if (!isPending) {
        ...
        form.onsubmit = async function (event) {
          ...
          if (save.disabled || !review || review.busy || review.requiresRefresh || review.conflict || !form.reportValidity() || !current(epoch, owner) || version !== formVersion) return;
