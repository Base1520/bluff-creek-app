(function () {
  ...
  async function loadAll(epoch) {
    ...
    var request = ++loadEpoch, owner = ++refreshOwner; refreshing = true;
    ...
    } finally { if (epoch === authEpoch && owner === refreshOwner) { refreshing = false; health(el("workspace-health-message").textContent, false); syncEditor(); ["workspace-refresh", "editor-refresh"].forEach(function (id) { if (el(id)) el(id).disabled = !!(draft && draft.busy); }); } }
  }
