(function(root){
  ...
  function create(options){
    var host=options.root,doc=host.ownerDocument,rows=[],loadId=0,selected='',previewURL=null,dialog=null,requestId=0,failed=false,ready=false,draft=null,loadedOwner=null;
    ...
    function closeDialog(force){if(force!==true&&draft&&(draft.pending||draft.uncertain||(draft.uploaded&&!draft.filed)))return;if(force!==true&&draft&&draft.initialForm&&formFingerprint()!==draft.initialForm&&!doc.defaultView.confirm(draft.filed?'Discard this review draft? The uploaded source page will remain in Documents.':'Discard this unsaved review? Your transcription and selected photo will be cleared.'))return;requestId++;draft=null;if(previewURL){URL.revokeObjectURL(previewURL);previewURL=null}if(dialog){if(dialog.open)dialog.close();dialog.remove();dialog=null}}
    function clear(){loadId++;rows=[];selected='';failed=false;ready=false;loadedOwner=null;closeDialog(true);find('membership-person').replaceChildren(new Option('All people',''));find('membership-search').value='';find('membership-history').replaceChildren();find('membership-status').textContent=''}
