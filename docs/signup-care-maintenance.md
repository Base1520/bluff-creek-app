...
Loading or retrying a saved profile now temporarily locks submission and repeated Retry. A pending older read cannot overwrite a newer successful submission. The profile read has a 15-second UI deadline; a timeout offers Retry without treating blank or unavailable data as a successful profile load, and a late response is ignored. Sign-out and account changes clear immediately; only the current account can release its loading controls. This does not cancel server work, change the verified-email requirement or activate public intake.
...
A person can have one plan per care role. Contacts are recorded against both person and role, so a teacher's contact does not reset a deacon's deadline. Successful contact restarts only that recurring role's interval; an attempt does not. A chosen next date survives later attempts without a new date. Another chosen date replaces it; a later successful contact starts a new interval unless it specifies its own next date. Older overrides cannot revive after that success. Future-dated contact entries do not prematurely count as completed contact. A one-time task completes after a successful contact on/after its start date, and restarting its start date excludes older scheduling history. Paused and completed plans remain visible.
...
The existing church project now has the complete **seven-file sequence** applied, including the final public profile-write pause. Both save_app_connection entry points deny browser execution even when frontend settings are present. Staff capability revision remains 20260907174301. Reopening requires a separately reviewed tracked migration and actual hosted signup/email/review acceptance; do not stop at the earlier six-file baseline or replay it into this project. See [hosted verification](office-hosted-verification-2026-09-08.md).

For a new isolated setup use the exact [preflight manifest](../tools/office-preflight/manifest.json), in order. Its current versions match hosted recorded history; all seven SQL hashes are preserved. Original filenames and historical six-file service/restore evidence remain in the [migration mapping](office-migrations.md).
