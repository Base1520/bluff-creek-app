...
After authorized church-owned backend setup, apply the base schema, the membership/care migration, then `supabase/migrations/20260908220623_office_content.sql`. See [schema contracts](office-content-schema.md). Keep current configuration blank until activation is approved. No existing committee lists, prayer requests or private decks were imported into code or the preview.
