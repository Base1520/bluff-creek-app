...
**Current setup — September 9:** the active manifest now requires eight migrations, including the already applied account guard at recorded version `20260909124122`. Fresh staff setup must include the guard; the existing church project must verify its eight recorded versions without replay. See [current guard alignment and limits](office-account-guard-hosted-2026-09-09.md). Original seven SQL hashes, maps and dated results below are historical and unchanged.
...
Run `node tools/office-preflight/cli.mjs` and `node --test tools/office-preflight/preflight.test.mjs` from the app root. Manifest format 3 requires the base migration first, all eight migrations in the inventory, unique timestamps, and ascending filenames matching declared dependencies. Unreviewed files, missing files, changed SQL, duplicate versions and out-of-order versions fail. The manifest explicitly identifies the sixth readiness source, seventh pause source and eighth account guard. The exact approved guard hash is independently pinned; a refreshed manifest digest cannot bless weakened guard SQL. The checker rejects a missing pause, omitted signature or added grant even if a local digest was refreshed. This checker remains read-only, local and credential-redacting.

Current preflight verification passes 23/23 tests and 45/45 local checks. The complete eight-file SQL test passes six checks covering unchanged records/readiness, both denied submission paths for anonymous/member/staff callers, PUBLIC inheritance, isolated existing-profile reads and preserved editor/admin review. These use fictional in-memory PostgreSQL only.
