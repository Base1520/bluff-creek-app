...
After separate church-backend authorization, follow the complete six-file setup, including the base schema, in the [reviewed preflight manifest](../tools/office-preflight/manifest.json) and [preflight guide](../tools/office-preflight/README.md#reviewed-sql-order). Use the [migration preparation and activation guide](office-migrations.md) to confirm the target schema and migration history before applying that sequence; do not replay a baseline into an existing deployment. See [base membership schema details](membership-schema.md) for the feature's data model. Verify real hosted staff roles, revocation, source-image access and uploads before any real records. Current roles are admin/editor/viewer; there is no assigned-deacon-only login role in this phase. Do not invite all deacons as broad editors without an explicit access decision.
