...
After separate church-backend authorization, apply the base schema and the three migrations in the explicit order documented in [signup/care setup](signup-care-maintenance.md); see [base membership schema details](membership-schema.md). Verify real hosted staff roles, revocation, source-image access and uploads before any real records. Current roles are admin/editor/viewer; there is no assigned-deacon-only login role in this phase. Do not invite all deacons as broad editors without an explicit access decision.
